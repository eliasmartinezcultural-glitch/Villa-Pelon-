...
function people(){return [...(V.life?.ambient||[]),...(V.life?.workers||[])].filter(p=>p&&p.id&&!p._v65Hidden)}
function buildingFor(p){const id=p.lifeBuildingId,b=V.buildingSystem?.byId?.get(id);return b||null}
function drawPerson(c,x,y,s=1,step=0){c.save();c.translate(x,y+Math.sin(phase()+step)*1.5);c.fillStyle='rgba(45,34,28,.92)';c.fillRect(-4*s,-11*s,8*s,9*s);c.fillRect(-5*s,-2*s,10*s,11*s);c.fillRect(-6*s,9*s,4*s,7*s);c.fillRect(2*s,9*s,4*s,7*s);c.fillStyle='rgba(226,190,115,.9)';c.fillRect(-3*s,-13*s,6*s,3*s);c.restore()}
function drawExteriorActivity(c){const B=V.buildingSystem;if(!B?.buildings)return;const t=phase();for(const b of B.buildings){const n=B.occupancy?.[b.id]||0;if(!n||!B.isOpen?.(b))continue;const d=b.door||{x:b.x+b.w/2,y:b.y+b.h};const type=String(b.type||'').toLowerCase();c.save();c.globalAlpha=.72;const pulse=(Math.sin(t+b.x*.01)+1)*.5;
...
function drawInteriorActivity(c,W,H){const B=V.buildingSystem,b=B?.inside;if(!b)return;const rooms=b.interior?.rooms||[],cols=Math.min(3,rooms.length||1),w=Math.min(820,W-42),h=Math.min(510,H-60),x=(W-w)/2,y=(H-h)/2,rw=(w-70-(cols-1)*12)/cols,t=phase();rooms.forEach((room,i)=>{const col=i%cols,row=Math.floor(i/cols),rx=x+28+col*(rw+12),ry=y+84+row*118,n=B.activities?.[b.id]?.[room]||0;if(!n)return;for(let k=0;k<Math.min(n,3);k++){const px=rx+35+(k%2)*55+Math.sin(t+k+i)*5,py=ry+65+(k>1?8:0);drawPerson(c,px,py,.72,k+i)}const label=String(room).toUpperCase();c.save();c.fillStyle='rgba(238,205,125,.15)';c.fillRect(rx+rw-55,ry+8,43,13);c.fillStyle='rgba(246,224,170,.9)';c.font='8px monospace';c.textAlign='center';c.fillText(`${n} EN ACCIÓN`,rx+rw-33,ry+17);c.restore()})}
function patch(){if(A.patched||!V.life?.drawOverlay)return;const old=V.life.drawOverlay;V.life.drawOverlay=function(c,w,h){old.call(this,c,w,h);const W=w||c.canvas.width,H=h||c.canvas.height;if(V.buildingSystem?.inside)drawInteriorActivity(c,W,H);else drawExteriorActivity(c)};A.patched=true;A.ready=true}
