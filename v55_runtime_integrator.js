/* VILLA PELÓN V56 — una sola capa narrativa de entrada; el juego comienza sin menú superpuesto. */
(()=>{'use strict';
const V=window.VillaPelon||(window.VillaPelon={}),state=V.gameState,life=V.life;
if(!state)return;
V.runtime=V.runtime||{};
V.runtime.version='V56';
V.runtime.platforms=['pc','mobile'];
V.runtime.worldBounds={w:3200,h:2000};
V.runtime.rules={singlePlayerState:true,singleLifeUpdate:true,roadsAreNavigation:true,housesStayOffRoads:true,noActorInsideBuilding:true,introNarrativeOnly:true};
...
if(start&&game&&!V.runtime.__intro){
  ...
  const next=()=>{
    i++;
    if(i<story.length){show();setTimeout(next,3300);return}
    state.started=false;
    start.classList.add('hidden');
    game.classList.remove('hidden');
    if(typeof V.loadGame==='function')V.loadGame();
    state.started=true;
    if(typeof V.startGame==='function')V.startGame();
  };
