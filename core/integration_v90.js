/* VILLA PELÓN — INTEGRATION 106.9
   Puente único entre motor, territorio, guardado y VERGEL.
   No crea una segunda fuente visual ni una segunda rutina de guardado.
   V106.9: nunca degrada la autoridad canónica 106.4 del mundo.
*/
(()=>{'use strict';
...
const WORLD={w:8200,h:4200,version:'106.4'};
...
if(G.worldRules)G.worldRules.version='106.4';
if(G.pixelArt)G.pixelArt.version='106.4';
function ensurePoint(id,fallback){let p=G.points.find(x=>x&&x.id===id);if(!p){p={id,...fallback};G.points.push(p)}return p}
function ensureTerritoryPoints(){
 ...
 G.clue=G.clue||{x:2040,y:430};G.jobSpot=G.jobSpot||{x:2200,y:700};
 if(!G.routes.some(r=>r?.id==='picada21_route'))G.routes.push({id:'picada21_route',points:[{x:4580,y:2120},{x:5400,y:2120},{x:6500,y:2120},{x:7550,y:2120},{x:7900,y:2120}],width:150,type:'rural_road'});
}
...
G.integrity={version:'106.9',worldSize:{w:WORLD.w,h:WORLD.h},picada21:{stop:{x:7550,y:2350},route:'picada21_route'},checkedAt:Date.now()};
V.worldApi=V.worldApi||{};V.worldApi.version='106.9';
V.worldApi.getPlayer=()=>{const s=S();return{x:+s.x||0,y:+s.y||0,day:+s.day||1,energy:+s.energy||0}};
V.worldApi.save=()=>{const b=document.getElementById('save');if(!b)return false;b.click();return true};V.saveGame=V.worldApi.save;
...
function harvestIfAvailable(e){const s=S();if(s.dialogue||!V.vergel?.inspect||!V.vergel?.harvest)return false;const info=V.vergel.inspect();if(!info?.ok)return false;const r=V.vergel.harvest();if(!r.ok){if(r.reason==='energia')toast('VERGEL · NECESITÁS MÁS ENERGÍA');else if(r.reason==='agotado')toast('VERGEL · RECURSO AGOTADO');return false}e?.preventDefault();e?.stopPropagation();toast('VERGEL · '+r.label+' +1');window.dispatchEvent(new CustomEvent('villa-pelon-vergel',{detail:r}));return true}
window.addEventListener('keydown',e=>{if(e.repeat)return;const k=e.key.toLowerCase();if(k==='e'||e.key===' '){harvestIfAvailable(e)}},true);
document.addEventListener('pointerdown',e=>{if(e.target?.id==='interact')harvestIfAvailable(e)},true);
window.addEventListener('villa-pelon-engine-ready',()=>{ensureTerritoryPoints();G.integrity.checkedAt=Date.now();if(V.engine?.setState)V.engine.setState('running');window.dispatchEvent(new CustomEvent('villa-pelon-world-ready',{detail:G.integrity}))});
})();