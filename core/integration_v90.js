/* VILLA PELÓN — INTEGRATION 93.1
   Puente único entre motor, territorio, guardado y VERGEL.
   Esta capa NO secuestra la interacción general: sólo consume E/ESPACIO
   cuando el jugador está realmente dentro de una zona VERGEL.
*/
(()=>{'use strict';
...
const WORLD={w:8200,h:4200,version:'93.1'};
...
function ensureTerritoryPoints(){
  ...
  if(!G.routes.length)G.routes=[{id:'picada21_route',points:[{x:4700,y:2185},{x:5600,y:2185},{x:6500,y:2185},{x:7500,y:2250}]}];
}
...
G.integrity={version:'93.1',worldSize:{w:WORLD.w,h:WORLD.h},picada21:{stop:{x:7550,y:2350},route:'picada21_route'},checkedAt:Date.now()};
V.worldApi=V.worldApi||{};
V.worldApi.version='93.1';
...
/* Sólo interceptamos teclado después de que el objetivo VERGEL confirme proximidad.
   No usamos capture/stopImmediatePropagation para no romper NPC, edificios ni misiones. */
