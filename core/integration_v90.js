...
const WORLD={w:8200,h:4200,version:'93.4'};
...
G.integrity={version:'93.4',worldSize:{w:WORLD.w,h:WORLD.h},picada21:{stop:{x:7550,y:2350},route:'picada21_route'},checkedAt:Date.now()};
V.worldApi=V.worldApi||{};
V.worldApi.version='93.4';
V.worldApi.getPlayer=()=>{const s=S();return{x:+s.x||0,y:+s.y||0,day:+s.day||1,energy:+s.energy||0}};
/* La única implementación real de guardado vive en el motor. Los servicios
   llaman al mismo botón/handler para no mantener dos formatos de save. */
V.worldApi.save=()=>{const b=document.getElementById('save');if(!b)return false;b.click();return true};
