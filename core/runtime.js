/* VILLA PELÓN — Runtime Core 1.0
   Autoridad estructural única para ciclo de vida, servicios y salud del motor.
   No dibuja, no crea un segundo game loop y no reemplaza el motor de juego.
*/
(()=>{
  'use strict';
  ...
  document.addEventListener('visibilitychange',()=>{if(document.hidden){E.pause();try{if(typeof V.saveGame==='function'&&V.gameState&&V.gameState.started)V.saveGame()}catch(err){console.warn('[Villa Pelón] autosave visibility',err)}}else if(V.gameState&&V.gameState.started)E.resume()});
  window.addEventListener('beforeunload',()=>{try{if(typeof V.saveGame==='function'&&V.gameState&&V.gameState.started)V.saveGame()}catch(_) {}});
  E.setState('ready');
  window.VILLA_PELON_ENGINE='1.0.0';
})();
