/* VILLA PELÓN — PRESENTATION RUNTIME V90.3 */
(()=>{'use strict';const V=window.VillaPelon||(window.VillaPelon={});
 function render(){const s=V.gameState,m=V.missions?.status?.(s);if(!s||!m)return;const q=document.getElementById('questText'),p=document.getElementById('questProgress');if(q)q.textContent=m.objective||m.title;if(p)p.textContent=m.total?'Paso '+Math.min(m.step+1,m.total)+' de '+m.total+' · Recompensa $'+m.reward:'Exploración libre · Ruta principal completada';const toast=document.getElementById('missionToast');if(toast){toast.textContent=s.missionPulse||'';toast.classList.toggle('show',!!s.missionPulse)}}
