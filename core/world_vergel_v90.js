/* VILLA PELÓN — VERGEL V90.4
   Subsistema de territorio/productividad.
   Estado persistente dentro de gameState para que guardado/recarga no desincronicen el mundo.
*/
(()=>{'use strict';
...
/* Año simple de 120 días: 30 días por estación. */
function seasonFor(day){return ['verano','otoño','invierno','primavera'][Math.floor(Math.max(0,day-1)/30)%4]}
...
VERGEL.harvest=function(){const S=state();sync();const z=nearest(S);if(!z)return{ok:false,reason:'lejos'};if((+S.energy||0)<4)return{ok:false,reason:'energia',label:z.label};const r=VERGEL.resources[key(z,VERGEL.day)];if(!r||!r.ready||r.amount<=0)return{ok:false,reason:'agotado',zone:z.id,label:z.label};r.amount--;addInventory(S,z.yield);S.vergelInventory=S.vergelInventory||{};S.energy=Math.max(0,(+S.energy||0)-4);S.stats=S.stats&&typeof S.stats==='object'?S.stats:{};S.stats.harvests=(+S.stats.harvests||0)+1;S.stats.vergelHarvests=(+S.stats.vergelHarvests||0)+1;S.flags=S.flags&&typeof S.flags==='object'?S.flags:{};S.flags.vergelUsed=true;return{ok:true,item:z.yield,label:z.yieldLabel,zone:z.id,left:r.amount,season:VERGEL.season,energy:S.energy}}
VERGEL.sync=sync;
