/* V87.11 — runtime de campaña integrado al único update del motor. */
(()=>{'use strict';
...
function state(){const s=S();if(!s)return null;s.campaign=s.campaign||{active:null,completed:[],records:[],visited:[],visitedNpc:[]};return s}
...
function check(){const s=state(),m=current();if(!s||!m||s.dialogue||s.transport)return false;
...
V.campaignRuntime={version:'V87.11.0',simpleObjectives:true,educational:true,check,targets:target,integratedUpdate:true};
})();