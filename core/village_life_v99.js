/* VILLA PELÓN V100 — VIDA DEL PUEBLO
   Extiende V99 con una sola simulación de vida autónoma.
   Recupera de V62 rutinas, destinos, clima contextual y movimiento.
   No crea motor, renderer ni guardado paralelo.
*/
(()=>{'use strict';
const V=window.VillaPelon||(window.VillaPelon={});
const S=V.gameState;if(!S)return;
const LIFE=V.villageLife=V.villageLife||{};LIFE.version='100.0';LIFE.events=LIFE.events||{};LIFE.history=Array.isArray(LIFE.history)?LIFE.history:[];
...
let last=performance.now();function frame(now){const dt=Math.min(.05,(now-last)/1000);last=now;if(S.started&&!S.dialogue){updatePeople(dt);updateAmbient(dt)}requestAnimationFrame(frame)}requestAnimationFrame(frame);
setInterval(()=>{try{tickEvents();persist()}catch(e){console.error('[VillaPelon][v100][life]',e)}},1000);
V.engine=V.engine||{};V.engine.health=V.engine.health||function(){return{ok:true}};V.engine.health.villageLife='100.0';
})();
