/* VILLA PELÓN — FUNCTIONAL HEALTHCHECK V95 */
(()=>{'use strict';
...
checks.push(['stability',V.stability?.version==='95.0'&&typeof V.stability?.normalize==='function']);
const failed=checks.filter(c=>!c[1]);V.health={version:'95.0',ok:failed.length===0,failed:failed.map(c=>c[0]),checkedAt:Date.now(),details:checks};
