/* VILLA PELÓN — FUNCTIONAL HEALTHCHECK V93
   Smoke-test del contrato del mundo y del render. No modifica gameplay.
*/
(()=>{'use strict';
...
function run(){const G=V.worldGeometry||{},S=V.gameState||{},M=V.missions||{},B=Array.isArray(G.buildings)?G.buildings:[],R=Array.isArray(G.roads)?G.roads:[];const checks=[];
 checks.push(['canvas',!!document.getElementById('world')]);checks.push(['renderOverlay',!!document.getElementById('worldDetail')&&!!V.renderCompositor?.singleRAF]);checks.push(['gameState',!!V.gameState&&typeof S.x==='number']);checks.push(['engine',!!V.engine]);checks.push(['world',V.world?.w===8200&&V.world?.h===4200]);
 ...
 const failed=checks.filter(c=>!c[1]);V.health={version:'93.0',ok:failed.length===0,failed:failed.map(c=>c[0]),checkedAt:Date.now(),details:checks};
