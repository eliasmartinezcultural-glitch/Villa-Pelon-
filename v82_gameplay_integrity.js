/* Villa Pelón V82 — capa transversal de jugabilidad.
   Normaliza teclado/táctil, pausa, menú, configuración y accesibilidad.
   No reemplaza game.js ni ui_system.js: los coordina.
*/
(()=>{'use strict';
...
function closeDialogue(){const d=$('dialogue');if(d){d.classList.add('hidden');if(S)S.dialogue=false}}
function openMenu(){const m=$('vpMenu');if(!m)return;m.classList.remove('hidden');if(S)S.dialogue=true;document.querySelectorAll('.touch button,.save').forEach(b=>b.blur());renderSettings()}
function closeMenu(){const m=$('vpMenu');if(!m)return;m.classList.add('hidden');if(S)S.dialogue=false}
...
function keyboard(){
 addEventListener('keydown',e=>{
  ...
  if(k==='escape'){e.preventDefault();toggleMenu();return}
  if(k==='m'){e.preventDefault();toggleMenu();return}
  if(k==='p'){e.preventDefault();toggleMenu();return}
  if(menuOpen())return;
 },{capture:true});
 addEventListener('blur',()=>{if(S){S.dialogue=!!$('dialogue')&&!$('dialogue').classList.contains('hidden')}});
}
function touchSafety(){document.querySelectorAll('[data-key]').forEach(b=>{b.addEventListener('contextmenu',e=>e.preventDefault());b.addEventListener('pointerdown',()=>b.classList.add('pressed'),{passive:true});['pointerup','pointercancel','pointerleave'].forEach(x=>b.addEventListener(x,()=>b.classList.remove('pressed'),{passive:true}))})}
...
function boot(){ensureMenu();keyboard();touchSafety();repairStart();setTimeout(renderSettings,0);V.gameplay82={version:'82.0.0',settings:cfg,pause:openMenu,resume:closeMenu};}
