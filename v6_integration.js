/* Villa Pelón V6.6 — INTEGRACIÓN FINAL
   Reconciliación de mundo, colisión, vida y guardado.
   La jugabilidad y la interacción permanecen bajo la autoridad existente.
*/
(()=>{'use strict';
const V=window.VillaPelon||(window.VillaPelon={});const X=V.v6Integration=V.v6Integration||{version:4,enabled:true,ready:false};const W=V.world;if(!W||!V.engine)return;
W.w=Math.max(Number(W.w)||0,8400);W.h=Math.max(Number(W.h)||0,5600);W.version=6;V.v6Map=V.v6Map||{version:3,width:W.w,height:W.h,river:{x:7000,y:0,w:1200,h:5600},regions:{city:{x:0,y:0,w:3900,h:3000},suburbs:{x:3500,y:400,w:2200,h:2800},rural:{x:3000,y:2700,w:4000,h:2900}}};V.v6Map.width=W.w;V.v6Map.height=W.h;
const river=V.v6Map.river;const bridges=[{y:790,h:150},{y:1330,h:150}];const inside=(x,y,b,pad=12)=>x>b.x-pad&&x<b.x+b.w+pad&&y>b.y-pad&&y<b.y+b.h+pad;const bridgeAt=y=>bridges.some(b=>Math.abs(y-(b.y+b.h/2))<b.h/2);const riverBlocked=(x,y)=>x>river.x-8&&x<river.x+river.w+8&&!bridgeAt(y);const solid=(x,y,ignore)=> (V.buildings||[]).some(b=>b!==ignore&&b.collision!==false&&inside(x,y,b,14));
...
function saveBridge(){const oldSave=V.story?.save;if(V.story&&!V.story.__v6SavePatched&&typeof oldSave==='function'){V.story.save=function(){const r=oldSave.apply(this,arguments);try{localStorage.setItem('villa_pelon_v6_state',JSON.stringify({version:4,x:V.state.x,y:V.state.y,day:V.state.day,minutes:V.state.minutes,money:V.state.money,energy:V.state.energy,inventory:V.state.inventory,history:V.state.history,relationships:V.state.relationships}))}catch(_){}return r};V.story.__v6SavePatched=true}}
