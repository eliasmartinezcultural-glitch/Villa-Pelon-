...
const expansion=[
 ...
 {x:2480,y:190,w:300,h:170,label:'VIVIENDA',type:'home'},
...
function drawRoads(c){
 roads.forEach(r=>{rect(c,r.x,r.y,r.w,r.h,'#6a5b4d');rect(c,r.x,r.y,r.w,3,'#8a7660');rect(c,r.x,r.y+r.h-3,r.w,3,'#4d443b');if(r.w>100){for(let x=r.x+18;x<r.x+r.w-12;x+=52)rect(c,x,r.y+r.h/2-2,26,4,'#b49a70')}});
 sidewalks.forEach(s=>rect(c,s.x,s.y,s.w,s.h,'#9a8a73'));
}
function drawTrees(c){trees.forEach(([x,y])=>{rect(c,x-5,y+13,10,15,'#654936');rect(c,x-16,y-5,32,20,'#35563d');rect(c,x-11,y-13,22,12,'#416847');rect(c,x-5,y-18,10,7,'#4d744f')})}
function drawSigns(c){const signs=[[95,570,'BARRIO NORTE'],[1125,170,'CALLE PRINCIPAL'],[2048,170,'CAMINO RURAL'],[680,1380,'SALIDA A CHACRAS']];signs.forEach(([x,y,t])=>{rect(c,x,y,4,28,'#594638');rect(c,x+4,y-3,Math.max(70,t.length*6),18,'#d1b47c');c.fillStyle='#2b3029';c.font='7px monospace';c.fillText(t,x+8,y+9)})}
const previousDraw=V.life&&V.life.drawWorld;
if(V.life&&typeof previousDraw==='function'&&!V.life.__v70Expansion){V.life.drawWorld=c=>{drawRoads(c);previousDraw(c);drawTrees(c);drawSigns(c)};V.life.__v70Expansion=true}
