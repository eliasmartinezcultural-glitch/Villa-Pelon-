...
(()=>{'use strict';
const V=window.VillaPelon||(window.VillaPelon={});const state=()=>V.state||{};const life=()=>V.life;const world=()=>V.world||{w:4200,h:2700};
const routes={
 ...
 escuela:[{x:760,y:815},{x:700,y:650},{x:575,y:485}],plaza:[{x:1000,y:815},{x:1120,y:760},{x:1250,y:815},{x:1000,y:900}],
...
V.v3Routes=routes;V.v3Places=places;
function citizen(id,name,role,home,work,route,color){return{id,name,role,home,work,route,color,x:home.x,y:home.y,index:0,target:null,speed:26,activity:'casa',moving:false}}
const citizens=[
 citizen('elvira','Doña Elvira','vecina',places.home,places.plaza,'plaza','#9b6651'),citizen('sergio','Sergio','repartidor',{x:900,y:570},places.shop,'avenida','#4c7190'),
 citizen('mabel','Mabel','productora',{x:1550,y:1000},places.rural,'rural','#7c6849'),citizen('cacho','Cacho','camionero',{x:1650,y:980},places.vineyard,'rural','#6b594d'),
 citizen('ariel','Ariel','jornalero',{x:2200,y:1500},places.rural,'chacra','#58744e'),citizen('norma','Norma','vecina',{x:3500,y:1800},places.nature,'rural','#876b78')
];
const traffic=[
 {id:'t1',type:'auto',route:'avenida',x:100,y:815,speed:72,index:1,color:'#536b72'},{id:'t2',type:'camioneta',route:'avenida',x:1900,y:815,speed:58,index:2,color:'#7b624c'},
 {id:'t3',type:'colectivo',route:'avenida',x:2850,y:815,speed:48,index:3,color:'#69715d'},{id:'t4',type:'tractor',route:'rural',x:1700,y:1500,speed:31,index:1,color:'#61733b'},
 {id:'t5',type:'camion',route:'rural',x:3000,y:1500,speed:42,index:3,color:'#77706a'},{id:'t6',type:'auto',route:'norteSur',x:1290,y:300,speed:55,index:1,color:'#536b72'},
 {id:'t7',type:'bicicleta',route:'plaza',x:1000,y:815,speed:34,index:1,color:'#435866'}
];
const animals=[
 {type:'vaca',x:1510,y:840,zone:{x1:1350,y1:790,x2:2250,y2:1420},vx:8,vy:3},{type:'vaca',x:1650,y:900,zone:{x1:1350,y1:790,x2:2250,y2:1420},vx:-6,vy:4},
 {type:'caballo',x:2050,y:1080,zone:{x1:1800,y1:950,x2:2350,y2:1450},vx:7,vy:-3},{type:'caballo',x:2150,y:1150,zone:{x1:1800,y1:950,x2:2350,y2:1450},vx:-5,vy:4},
 {type:'gallina',x:940,y:560,zone:{x1:850,y1:490,x2:1450,y2:660},vx:14,vy:5},{type:'gallina',x:1030,y:575,zone:{x1:850,y1:490,x2:1450,y2:660},vx:-10,vy:4},{type:'gallina',x:1360,y:550,zone:{x1:850,y1:490,x2:1450,y2:660},vx:8,vy:-6}
];
V.v3Citizens=citizens;V.v3Traffic=traffic;V.v3Animals=animals;
...
if(L){const oldUpdate=L.update,oldDraw=L.drawWorld;L.v3=true;L.v3Routes=routes;L.v3Citizens=citizens;L.v3Traffic=traffic;L.v3Animals=animals;
 L.update=(dt,minutes)=>{if(typeof oldUpdate==='function')oldUpdate(dt,minutes);const h=(minutes||480)/60;citizens.forEach(n=>{const a=schedule(n,h);if(a!==n.activity){n.activity=a;n.target=null;n.index=0}const t=destination(n,a),d=Math.hypot(t.x-n.x,t.y-n.y);if(d>38){if(a==='casa')n.target=t;else if(!n.target||Math.hypot(n.target.x-n.x,n.target.y-n.y)<30)n.target=routeTarget(routes[n.route]||routes.avenida,n.x,n.y);const q=n.target,dx=q.x-n.x,dy=q.y-n.y,dd=Math.hypot(dx,dy)||1,step=Math.min(dd,n.speed*dt);n.x+=dx/dd*step;n.y+=dy/dd*step;n.moving=true;if(Math.hypot(t.x-n.x,t.y-n.y)<95)n.target=t}else n.moving=false});traffic.forEach(v=>moveRoute(v,dt));animals.forEach(a=>{a.x+=a.vx*dt;a.y+=a.vy*dt;if(a.x<a.zone.x1||a.x>a.zone.x2)a.vx*=-1;if(a.y<a.zone.y1||a.y>a.zone.y2)a.vy*=-1});L.eventText=eventFor(h)};
 L.drawWorld=(ctx)=>{if(typeof oldDraw==='function')oldDraw(ctx);traffic.forEach(v=>vehicle(ctx,v));citizens.forEach(n=>person(ctx,n));animals.forEach(a=>animal(ctx,a))};}
...
function person(c,n){shadow(c,n.x,n.y,13);c.save();c.translate(n.x,n.y);const b=n.moving?Math.sin(performance.now()/100+n.x)*2:0;c.fillStyle=n.color;c.fillRect(-9,-1+b,18,23);c.fillStyle='#e0ad88';c.beginPath();c.arc(0,-12+b,10,0,Math.PI*2);c.fill();c.fillStyle='#342c28';c.fillRect(-9,-22+b,18,5);c.restore()}
