/* Villa Pelón V4 — CAPA VISUAL Y AMBIENTAL
   Refuerza el mundo existente sin duplicar el motor: lectura espacial,
   actividad, clima, partículas y una identidad visual claramente V4.
*/
(()=>{'use strict';
const V=window.VillaPelon||(window.VillaPelon={}),start=performance.now();
function draw(){
  const e=V.engine;if(!e||!e.canvas||!e.ctx||!V.state)return;
  const ctx=e.ctx,t=(performance.now()-start)/1000,s=V.state;
  ...
  const hour=s.minutes/60;let alpha=hour<6||hour>21?.18:hour<8?.07:hour>19?.08:0;
  if(alpha){ctx.fillStyle=`rgba(20,31,48,${alpha})`;ctx.fillRect(0,0,w,h)}
  const g=ctx.createRadialGradient(w/2,h/2,Math.min(w,h)*.28,w/2,h/2,Math.max(w,h)*.72);g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(1,'rgba(12,15,11,.24)');ctx.fillStyle=g;ctx.fillRect(0,0,w,h);
  ctx.globalAlpha=.25;for(let i=0;i<18;i++){const x=(i*137+t*(8+i%4)*3)%w,y=(i*83+Math.sin(t*.7+i)*22+t*4)%h;ctx.fillStyle='#f0dca8';ctx.fillRect(x,y,1.5,1.5)}ctx.globalAlpha=1;
  const n=e.nearest?.();if(n&&n.x!=null){ctx.fillStyle='rgba(25,31,23,.86)';ctx.fillRect(w/2-115,h-92,230,32);ctx.strokeStyle='rgba(230,194,110,.55)';ctx.strokeRect(w/2-115,h-92,230,32);ctx.fillStyle='#f2ddb0';ctx.font='bold 12px system-ui';ctx.textAlign='center';ctx.fillText('E · '+(n.name||n.label||'EXPLORAR').toUpperCase(),w/2,h-71)}
  ctx.restore();
}
...
const wait=()=>install()||requestAnimationFrame(wait);wait();V.v4Visual={version:4,install};
})();