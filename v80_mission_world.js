...
const aliases={'escuela':'escuela','school':'escuela','radio':'radio','almacen':'almacen','shop':'almacen','galpon':'galpon','bodega':'bodega','chacra':'las_chacras','plaza':'plaza','rio':'rio','canal_viejo':'canal_viejo','las_chacras':'las_chacras','los_vinedos':'los_vinedos'};
...
function norm(v){return aliases[String(v||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')]||String(v||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')}
function mark(step){init();const r=rt();if(!r?.mark)return false;const activeBefore=r.active;const ok=r.mark(step,activeBefore);if(ok){M.recent={step,mission:activeBefore,at:Date.now()};persist();render();if(V.worldEvent)V.worldEvent('mission-progress',{step,mission:activeBefore})}return ok}
function visit(place){const p=norm(place);return [`visitar:${p}`,`visitar:${String(place||'').toLowerCase()}`].some(step=>mark(step))}
function talk(name){return mark(`hablar:${norm(name)}`)}
function object(id){const raw=norm(id);const map={archivo:'buscar:archivo_escuela',foto:'encontrar:foto_escolar',foto_escolar:'encontrar:foto_escolar',herramientas:'tomar_herramienta',cajones:'entregar:cajon',cosecha:'hacer:cosecha',pan:'observar:pan',acequia:'observar:acequia',compuerta:'observar:compuerta',bodega_la_ribera:'observar:bodega_la_ribera',mapa:'observar:mapa'};const step=map[raw];return step?mark(step):false}
...
function rewardOnComplete(){const r=rt();if(!r)return;const completed=r.completed||[];const key='villa_pelon_rewarded_v80';let done=[];try{done=JSON.parse(localStorage.getItem(key)||'[]')}catch(_){};completed.forEach(mid=>{if(done.includes(mid))return;const m=r.mission?.(mid);if(!m?.reward)return;const g=V.gameState;if(g&&Number.isFinite(m.reward.money))g.money+=m.reward.money;if(m.reward.item&&typeof V.addItem==='function')V.addItem(m.reward.item);done.push(mid);if(V.worldEvent)V.worldEvent('mission-reward',{mission:mid,reward:m.reward})});try{localStorage.setItem(key,JSON.stringify(done))}catch(_){};render()}
function onEvent(e){const d=e.detail||{};switch(d.type){case'spatial-enter':case'building-enter':visit(d.room||d.building);break;case'interior-object':object(d.object);break;case'object-inspect':object(d.object);break}rewardOnComplete();persist()}
function wrapDialogue(){if(V.__v80DialogueWrapped||typeof V.openDialogue!=='function')return;V.__v80DialogueWrapped=true;const original=V.openDialogue;V.openDialogue=function(speaker,lines){const result=original.apply(this,arguments);if(String(speaker||'').trim())talk(speaker);rewardOnComplete();persist();return result}}
function render(){const el=document.getElementById('questText'),m=current();if(!el||!m)return;const r=rt(),steps=r?.steps?.[m.id]||[];el.textContent=`${m.title} · ${steps.length}/${m.steps?.length||0}`}
