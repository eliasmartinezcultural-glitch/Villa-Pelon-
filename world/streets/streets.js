...
S.version=6.27;S.singleAuthority=true;
...
S.roads=roads;S.roadById=Object.fromEntries(roads.map(r=>[r.id,r]));S.urbanRoads=roads.filter(r=>r.kind==='urban'||r.kind==='urban-edge');S.productiveRoads=roads.filter(r=>r.kind==='productive');S.ruralRoads=roads.filter(r=>r.kind==='rural'||r.kind==='service');
...
function contains(r,x,y,pad=0){return x>=r.x-pad&&x<=r.x+r.w+pad&&y>=r.y-pad&&y<=r.y+r.h+pad}S.onRoad=(x,y)=>roads.some(r=>contains(r,x,y));S.roadAt=(x,y)=>roads.find(r=>contains(r,x,y))||null;S.onSidewalk=(x,y)=>roads.some(r=>{const o={x:r.x-S.sidewalkWidth,y:r.y-S.sidewalkWidth,w:r.w+S.sidewalkWidth*2,h:r.h+S.sidewalkWidth*2};return contains(o,x,y)&&!contains(r,x,y)});S.nearestRoad=(x,y)=>{let best=null,bd=Infinity;for(const r of roads){const cx=Math.max(r.x,Math.min(x,r.x+r.w)),cy=Math.max(r.y,Math.min(y,r.y+r.h)),d=Math.hypot(x-cx,y-cy);if(d<bd){bd=d;best=r}}return best};S.isRuralRoad=(x,y)=>{const r=S.roadAt(x,y);return !!r&&(r.kind==='rural'||r.kind==='service'||r.kind==='productive')};S.isUrbanRoad=(x,y)=>{const r=S.roadAt(x,y);return !!r&&(r.kind==='urban'||r.kind==='urban-edge')};
...
function crosswalk(c,x,y,w,h){c.fillStyle='rgba(233,223,192,.9)';for(let i=0;i<8;i++)c.fillRect(Math.round(x+i*w/8),Math.round(y),Math.max(6,Math.round(w/16)),Math.round(h))}function lamp(c,x,y){c.fillStyle='#4d4439';c.fillRect(Math.round(x-2),Math.round(y),4,42);c.beginPath();c.arc(Math.round(x),Math.round(y),6,0,Math.PI*2);c.fill();c.fillStyle='#e9d79d';c.beginPath();c.arc(Math.round(x),Math.round(y-4),4,0,Math.PI*2);c.fill()}function sign(c,x,y,text){c.fillStyle='#5a4a39';c.fillRect(Math.round(x-2),Math.round(y),4,30);c.fillStyle='#d8cda9';c.fillRect(Math.round(x-34),Math.round(y-18),68,20);c.fillStyle='#4b4034';c.font='8px monospace';c.textAlign='center';c.fillText(text,x,y-4);c.textAlign='left'}
S.renderOverlay=(c)=>{for(const r of S.urbanRoads){const px=r.orientation==='horizontal'?r.x+r.w*.5:r.x-42;const py=r.orientation==='horizontal'?r.y-46:r.y+r.h*.5;lamp(c,px,py)}for(const p of S.intersections){const a=S.roadById[p.a],b=S.roadById[p.b];if(!a||!b||!S.isUrbanRoad(p.x,p.y))continue;const h=a.orientation==='horizontal'?a:b,v=a.orientation==='vertical'?a:b;crosswalk(c,p.x-52,p.y-v.h*.5,104,18);crosswalk(c,p.x-52,p.y+v.h*.5-18,104,18);crosswalk(c,p.x-h.w*.5,p.y-52,18,104);crosswalk(c,p.x+h.w*.5-18,p.y-52,18,104);sign(c,p.x-55,p.y-68,a.name)}};
S.audit=()=>({version:S.version,roadCount:roads.length,urbanCount:S.urbanRoads.length,productiveCount:S.productiveRoads.length,ruralCount:S.ruralRoads.length,intersectionCount:S.intersections.length,singleAuthority:true,derivedOverlay:true});V.v4?.register?.('streets',S);
})();