/* VILLA PELÓN V54 — MASTER WORLD: topología coherente y acabado pixel-art. */
(()=>{'use strict';
...
function rect(c,x,y,w,h,f){c.fillStyle=f;c.fillRect(x|0,y|0,w|0,h|0)}
function stroke(c,x1,y1,x2,y2,f,w=1){c.strokeStyle=f;c.lineWidth=w;c.beginPath();c.moveTo(x1,y1);c.lineTo(x2,y2);c.stroke()}
function road(c,r){rect(c,r.x,r.y,r.w,r.h,r.kind==='rural'?'#9b8867':'#50564f');if(r.kind!=='rural'){rect(c,r.x,r.y,r.w,4,'#bdb69d');rect(c,r.x,r.y+r.h-4,r.w,4,'#bdb69d')}if(r.kind==='avenue')for(let x=r.x+18;x<r.x+r.w;x+=78)rect(c,x,r.y+r.h/2-2,38,4,'#d9d0ad');if(r.kind==='street')for(let y=r.y+18;y<r.y+r.h;y+=78)rect(c,r.x+r.w/2-2,y,4,38,'#d9d0ad');if(r.kind==='rural'){stroke(c,r.x,r.y+2,r.x+r.w,r.y+2,'#b9a47d',2);stroke(c,r.x,r.y+r.h-2,r.x+r.w,r.y+r.h-2,'#806f55',2)}}
function sidewalk(c,r){if(r.kind==='rural'||r.kind==='river')return;if(r.w>r.h){rect(c,r.x,r.y-10,r.w,10,'#c7bea4');rect(c,r.x,r.y+r.h,r.w,10,'#c7bea4')}else{rect(c,r.x-10,r.y,10,r.h,'#c7bea4');rect(c,r.x+r.w,r.y,10,r.h,'#c7bea4')}}
...
function crossings(c){[[455,267],[1120,267],[1822,267],[2490,267],[2670,267],[455,1027],[1120,1027],[1822,1027],[2490,1027],[2670,1027]].forEach(([x,y])=>{for(let i=-30;i<=30;i+=12){rect(c,x+i,y-24,7,7,'#e0d6b5');rect(c,x+i,y+17,7,7,'#e0d6b5')}})}
function draw(c){ground(c);roads.forEach(r=>road(c,r));roads.forEach(r=>sidewalk(c,r));crossings(c);river(c);plaza(c);[[120,150],[900,150],[1460,150],[2200,150],[120,1180],[1460,1180],[2300,1180]].forEach(p=>pole(c,...p));[[970,550],[1320,550],[2020,550],[600,1160],[1510,1160],[2580,730]].forEach(p=>lamp(c,...p));[[130,350],[930,370],[1490,430],[2160,360],[300,900],[1020,900],[1510,900],[2200,840],[2580,1510]].forEach(p=>tree(c,...p));fence(c,1940,1180,430,250);fence(c,2410,1160,420,270)}
if(!life.__v54Master){life.__v54Master=true;const old=life.drawWorld;life.drawWorld=function(c){draw(c);if(old)old(c)}}
V.worldMaster={version:'V54',roads,riverX:RIVER,homesOutsideRoads:homes.every(h=>!roads.some(r=>overlap(h,r,2))),worldBounds:{w:W,h:H}};
})();