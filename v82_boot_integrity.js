/* VILLA PELÓN V82 — BOOT ÚNICO + DIAGNÓSTICO
   Una sola autoridad de arranque. V78/V81 quedan fuera de la cadena.
*/
(()=>{'use strict';
const V=window.VillaPelon||(window.VillaPelon={});
const bootError=(message,error)=>{
 let box=document.getElementById('v82Error');
 if(!box){box=document.createElement('div');box.id='v82Error';box.style.cssText='position:fixed;inset:12px;z-index:9999;background:#160f0d;color:#f4ead2;border:2px solid #b95e4e;padding:18px;font:14px/1.5 system-ui,sans-serif;overflow:auto;white-space:pre-wrap'}
 box.textContent='VILLA PELÓN · DIAGNÓSTICO V82\n\n'+message+(error?'\n\n'+(error.stack||error.message||error):'');
};
window.addEventListener('error',e=>{if(!V.gameState)bootError('El motor no llegó a inicializarse.',e.error||e.message)},true);
...
function boot(){
 ...
 if(!start||!game||!btn||!card||!s){bootError('Faltan elementos esenciales: '+JSON.stringify({start:!!start,game:!!game,button:!!btn,card:!!card,state:!!s}));return false}
 /* V72/V78/V81 ya no controlan la intro: esta capa reconstruye el botón una sola vez. */
 ...
 const enter=()=>{try{
  ...
  if(saved&&typeof saved==='object'){const safe=['x','y','speed','money','energy','minutes','day','quest','inventory','walk','facing','v74Quest','v74Seen','historyQuest','historySeen'];safe.forEach(k=>{if(Object.prototype.hasOwnProperty.call(saved,k))s[k]=saved[k]})}
  s.started=true;s.dialogue=false;s.saved=false;start.classList.add('hidden');game.classList.remove('hidden');
  const q=document.getElementById('questText');if(q)q.textContent='01/21 · Llegar y observar · INTERACTUÁ';
  V.__booted=true;V.audit=V.audit||{};V.audit.v82={boot:'single',buttonBound:true,saveRecovery:true,gameState:true};window.dispatchEvent(new CustomEvent('villa-pelon-started'));
 }catch(error){bootError('No se pudo iniciar la partida.',error)}};
 if(!current.__v82Bound){current.addEventListener('click',enter,{capture:true});current.__v82Bound=true}
 V.bootGame=enter;return true;
}
